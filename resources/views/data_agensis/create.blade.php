...
<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-bold text-base text-gray-800">Tambah Data Agensi</h2>
    ...
    <div class="py-6">
        <div class="max-w-3xl mx-auto px-4">
            <div class="bg-white shadow rounded-lg p-6">
                ...
                <form action="{{ route('data-agensis.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf

                    {{-- === SECTION A: Identitas Agensi === --}}
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="id_agensis" id="id_agensis"
                            value="{{ old('id_agensis', $generatedId) }}"
                            class="w-full bg-gray-100 border-gray-300 rounded-md shadow-sm" readonly>
                    </div>

                    {{-- Hidden input untuk JS validasi NIP --}}
                    <input type="hidden" id="id_agensis" value="{{ old('id_agensis', $generatedId) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nama_agensi" value="{{ old('nama_agensi') }}" required
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="alamat_agensi" rows="3" required
                            class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ old('alamat_agensi') }}</textarea>
                    </div>

                    {{-- Telp Agensi --}}
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Telp Agensi</label>
                        <input type="text" name="telp_agensi" value="{{ old('telp_agensi') }}" required
                            class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="email_agensi" id="email_agensi" value="{{ old('email_agensi') }}" 
                            class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">

                        <p id="error_email_agensi" class="text-red-500 text-sm mt-1 hidden">Email harus mengandung simbol @.</p>
                    </div>
                   ...
                   {{-- Badan Usaha --}}
                    <div class="mb-4">
                        ...
                        <select name="badan_usaha" id="badan_usaha" required
                            class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            <option value="">-- Pilih --</option>
                            @foreach(['PT', 'CV', 'UD', 'Perorangan'] as $value)
                                <option value="{{ $value }}" {{ old('badan_usaha') == $value ? 'selected' : '' }}>{{ $value }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-4 hidden">
                        ...
                        <select name="dokumen_legasitas" id="dokumen_legasitas" 
                            class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            <option value="">-- Pilih --</option>
                            @foreach(['Terlampir', 'Tidak Terlampir'] as $value)
                                <option value="{{ $value }}" {{ old('dokumen_legasitas') == $value ? 'selected' : '' }}>{{ $value }}</option>
                            @endforeach
                        </select>
                    </div>

                    {{-- Kontrak PSM --}}
                    <div class="mb-6" id="upload_dokumen_div">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Upload Dokumen Legalitas (PDF)</label>
                        <input type="file" name="kontrak_psm" id="kontrak_psm" accept="application/pdf" required
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nama_pic_utama" id="nama_pic_utama" value="{{ old('nama_pic_utama') }}" 
                            class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">

                        <p id="error_nama_pic_utama" class="text-red-500 text-sm mt-1 hidden">Nama PIC Utama hanya boleh huruf dan spasi.</p>
                    </div>


                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Telp PIC Utama</label>
                        <input type="text" name="telp_pic_utama" value="{{ old('telp_pic_utama') }}" 
                            class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                    </div>

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Nama Wakil PIC</label>
                        <input type="text" name="nama_wakil_pic" id="nama_wakil_pic" value="{{ old('nama_wakil_pic') }}"
                            class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">

                        <p id="error_nama_wakil_pic" class="text-red-500 text-sm mt-1 hidden">Nama Wakil PIC hanya boleh huruf dan spasi.</p>
                    </div>


                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Telp Wakil PIC</label>
                        <input type="text" name="telp_wakil_pic" value="{{ old('telp_wakil_pic') }}" 
                            class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                    </div>
