...
<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="fw-bold text-primary mb-0">Buat Pembayaran Payroll Karyawan</h2>
    ...
    <div class="py-12">
        <div class="card shadow border-0 rounded-3">
            <div class="card-header bg-gradient-primary text-white py-3 d-flex justify-content-between align-items-center">
                <h5 class="mb-0"><i class="bi bi-cash-coin me-2"></i> Formulir Pembayaran Payroll</h5>
            </div>

            <div class="card-body bg-light">
                <form action="{{ route('pembayaran-payroll.store') }}" method="POST" id="payrollForm">
                    @csrf
                   ...
                   <div class="mb-4 p-4 bg-white rounded shadow-sm">
                    ...
                    <div class="mb-4 p-3 bg-white rounded shadow-sm">
                        ...
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <input type="text" id="nama_bank" class="form-control bg-light" readonly>
                            </div>
                            <div class="col-md-4">
                                <label>Nama Rekening</label>
                                <input type="text" id="nama_rekening" class="form-control bg-light" readonly>
                            </div>
                            <div class="col-md-4">
                                <label>Nomor Rekening</label>
                                <input type="text" id="nomor_rekening" class="form-control bg-light" readonly>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="col-md-6 border-end pe-4">
                                ...
                                <div class="mt-4">
                                    ...
                                    <input type="text" id="total_earning" name="total_earning"
                                        class="form-control bg-success-subtle fw-bold text-end" readonly>
                                </div>
                            </div>
                               ...
                               @foreach ([
                                ...
                                <div class="mt-4">
                                    ...
                                    <input type="text" id="total_deduction" name="total_deduction"
                                        class="form-control bg-danger-subtle fw-bold text-end" readonly>
                                </div>
                    ...
                    <div class="text-end mt-4">
                        <button class="btn btn-primary px-5 py-2">
                            <i class="bi bi-save me-1"></i> Simpan Payroll
                        </button>
                    </div>
                </form>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const karyawanInput = document.getElementById('karyawan_id');

    payrollSelect.addEventListener('change', async function() {
        const selected = this.options[this.selectedIndex];
        const karyawanId = selected.dataset.karyawan_id;
        if (!karyawanId) return;

        karyawanInput.value = karyawanId;

        // isi data default
        basic_salary.value = toRupiah(selected.dataset.basic_salary || 0);
        leader_fee.value = toRupiah(selected.dataset.leader_fee || 0);
        insentive_fee.value = toRupiah(selected.dataset.insentive_fee || 0);

        nama_bank.value = selected.dataset.nama_bank || '';
        nama_rekening.value = selected.dataset.nama_rekening || '';
        nomor_rekening.value = selected.dataset.nomor_rekening || '';

        const bulan = bulan_payroll.value;
        penalties.value = '0';
        loan_repayment.value = '0';
        remaining_loan.value = '0';

        // ======= FETCH PENALTIES =======
        if (bulan) {
            try {
                const res = await fetch(`/pembayaran-payrolls/get-penalties/${karyawanId}/${bulan}`);
                const data = await res.json();
                const penaltyVal = Math.round(Number(data.penalties || 0));
                penalties.value = toRupiah(penaltyVal);
            } catch (err) {
                penalties.value = '0';
            }
        }
        ...
        // ======= FETCH LOAN =======
        try {
            const resLoan = await fetch(`/pembayaran-payrolls/get-loan-data/${karyawanId}`);
            const dataLoan = await resLoan.json();
            const repay = Math.round(Number(dataLoan.loan_repayment || 0));
            const remain = Math.round(Number(dataLoan.remaining_loan || 0));
            loan_repayment.value = toRupiah(repay);
            remaining_loan.value = toRupiah(remain);
        } catch (err) {
            loan_repayment.value = '0';
            remaining_loan.value = '0';
        }
